...
CollateData <- function(Experiment, reference_path, output_path, IRMode = c("SpliceOverMax", "SpliceMax"), 
  localHub = FALSE, BPPARAM = BiocParallel::bpparam()) {
    ...
    junc.list = suppressWarnings(BiocParallel::bplapply(NxtIRF.SplitVector(seq_len(nrow(df.internal)), BPPARAM$workers),
      function(work, df.internal, norm_output_path) {
        suppressPackageStartupMessages({
          library(data.table)
          library(stats)
        })
        ...
        # Write temp file
            fst::write.fst(as.data.frame(junc), 
                file.path(norm_output_path, paste(block$sample[i], "junc.fst.tmp", sep=".")))        
        }
        return(junc.segment)
      }, df.internal = df.internal, norm_output_path, BPPARAM = BPPARAM
    ))
    junc.common = NULL
    ...
    irf.list = suppressWarnings(BiocParallel::bplapply(NxtIRF.SplitVector(seq_len(nrow(df.internal)), BPPARAM$workers),
      function(work, df.internal, norm_output_path, runStranded, semi_join.DT) {
        suppressPackageStartupMessages({
          library(data.table)
          library(stats)
        })
        ...
        # Compile IRFinder based on strand
          ...
          fst::write.fst(as.data.frame(irf), 
            file.path(norm_output_path, paste(block$sample[i], "irf.fst.tmp", sep=".")))
        }
        return(irf.segment)
      }, df.internal = df.internal, norm_output_path, 
...
# Reassign +/- based on junctions.fst annotation
  ...
  message("Generating NxtIRF FST files")
    suppressWarnings(BiocParallel::bplapply(NxtIRF.SplitVector(seq_len(nrow(df.internal)), BPPARAM$workers),
      function(work, df.internal, junc.common, irf.common, Splice.Anno, norm_output_path) {
        suppressPackageStartupMessages({
          library(data.table)
          library(stats)
        })
        ...
        for(i in seq_len(length(work))) {
        junc = as.data.table(
            fst::read.fst(file.path(norm_output_path, paste(block$sample[i], "junc.fst.tmp", sep=".")))
        )
        junc[, start := start + 1]
        junc$strand = NULL
        ...
        irf = as.data.table(
            fst::read.fst(file.path(norm_output_path, paste(block$sample[i], "irf.fst.tmp", sep=".")))
        )
        irf[, start := start + 1]
        ...
        file.remove(file.path(norm_output_path, paste(block$sample[i], "junc.fst.tmp", sep=".")))
        file.remove(file.path(norm_output_path, paste(block$sample[i], "irf.fst.tmp", sep=".")))
      }
    }, df.internal = df.internal, junc.common = junc.common, irf.common = irf.common, 
  ...
  # Build counts and filter data here to avoid loading times later
  colData = Experiment
  irf_fst_files = file.path(norm_output_path, paste(colData$sample, "irf.fst", sep="."))
  
  if(!dir.exists(file.path(output_path, "Collated"))) dir.create(file.path(output_path, "Collated"))
  se.list = BuildSE(irf_fst_files, colData, IRMode = IRMode, 
    fst_path = file.path(output_path, "Collated"),
    BPPARAM = BPPARAM)
  
  saveRDS(se.filter, file.path(norm_output_path, "se.data.Rds"))
