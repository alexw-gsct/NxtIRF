...
#'      use_subdir = FALSE)
#'  # is equivalent to:
#'  df = FindSamples(sample_path = './IRFinder_output', 
#'      suffix = c(".txt.gz", ".cov"),
#'      c("irf_file", "cov_file"),
#'       use_subdir = FALSE)
#' @md
...
#'
#' @return A NxtIRF SummarizedExperiment (`NxtSE`) object containing the following assays:
...
#'
#' # use colData of selected samples:
#' colData = data.frame(
#'     samples = c("UT1","UT2", "Rx1", "Rx2"),
...
MakeSE = function(collate_path, colData, RemoveOverlapping = TRUE) {
    ...
    files.todo = file.path(normalizePath(collate_path), paste(item.todo, "fst", sep="."))
    assert_that(all(file.exists(files.todo)),
    msg = "FST File generation appears incomplete. Suggest run CollateData() again")

    assert_that(file.exists(file.path(collate_path, "colData.Rds")),
