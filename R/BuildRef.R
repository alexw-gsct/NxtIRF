...
#' print(records)
#'
#' ## Run BuildReference using 'AH65745' as human genome, and 'AH64631' as gene annotations:
#'\donttest{
#' BuildReference(fasta = "", gtf = "",
#'     ah_genome = "AH65745", ah_transcriptome = "AH64631",
#'     reference_path = "./Reference",
#'     genome_type = "hg38")
#' }
#' 
#' ## Run BuildReference using custom supplied FASTA and GTF files as 
#' ##   Ensembl FTP links:
#'\donttest{
#' BuildReference(
#'     fasta = "ftp://ftp.ensembl.org/pub/release-75/fasta/homo_sapiens/dna/Homo_sapiens.GRCh37.75.dna.primary_assembly.fa.gz", 
#'     gtf = "ftp://ftp.ensembl.org/pub/release-75/gtf/homo_sapiens/Homo_sapiens.GRCh37.75.gtf.gz",
#'     reference_path = "./Reference",
#'     genome_type = "hg19")
#' }
#' @seealso [GenerateMappabilityReads()], [GenerateMappabilityBED()], [AnnotationHub()]
#' @md
#' @export
BuildReference <- function(fasta = "genome.fa", gtf = "transcripts.gtf",
    ...
    assert_that(
        any(as.character(GenomicRanges::seqnames(gtf_gr)) %in% chrOrder),
        msg = paste(
...
.fetch_fasta <- function(reference_path = "./Reference",
    ...
    if (ah_genome != "") {
        ...
        assert_that(file.exists(.parse_valid_file(fasta_file)),
            msg = paste(
...
.fetch_gtf <- function(reference_path = "./Reference",
    ...
    if (ah_transcriptome != "") {
        ...
        assert_that(file.exists(gtf_file),
            msg = paste(
                "Given transcriptome gtf file", gtf,
                "not found"
            )
        )
...
.fetch_AH <- function(ah_record_name, localHub = FALSE,
    ...
    assert_that(substr(ah_record_name, 1, 2) == "AH",
        msg =
            paste(
                ah_record_name,
    ...
    assert_that(ah_record_name %in% names(ah),
        msg =
            paste(
                ah_record_name,
...
.parse_valid_file <- function(file, msg = "") {
    if (!is_valid(file)) {
        ...
        return("")
    } else if (nchar(file) > 8 && (
            substr(file, 1, 8) == "https://" |
            substr(file, 1, 6) == "ftp://"
        )) {
        url <- file
...
.process_introns <- function(reference_path, genome, UseExtendedTranscripts) {
    ...
    candidate.introns[as.data.table(Transcripts),
        on = "transcript_id",
    ...
    if ("protein_id" %in% names(mcols(Proteins))) {
        candidate.introns[as.data.table(Proteins),
            on = "transcript_id",
            c("protein_id") := list(get("i.protein_id"))
        ]
    }
    if ("ccds_id" %in% names(mcols(Exons))) {
        candidate.introns[as.data.table(Exons),
            on = "transcript_id",
...
.gen_irf <- function(reference_path, extra_files, genome,
    ...
    # Dir
    if (length(exclude.omnidirectional) > 0) {
        introns.intersect.dir <- GenomicRanges::intersect(
            introns.unique.dir,
            c(
                exclude.omnidirectional,
                .grDT(exclude.directional)
    ...
    tmpdir.IntronCover.summa[as.data.table(introns.unique),
        on = "intron_id",
        c(
            "seqnames", "intron_start", "intron_end",
    ...
    tmpnd.IntronCover.summa[
        ...
        c("IRFname") := paste(get("IRFname"),
            "known-exon+anti-over+anti-near",
            sep = "/"
        )
    ]
    tmpnd.IntronCover.summa[
    ...
    write.fst(tmpnd.IntronCover.summa, file.path(
        reference_path, "fst",
        "Introns.ND.fst"
    ))
    write.fst(tmpdir.IntronCover.summa, file.path(
        reference_path, "fst",
        "Introns.Dir.fst"
    ))
    ...
    if ("gene_biotype" %in% names(mcols(Transcripts))) {
        rRNA <- as.data.frame(Transcripts[grepl(
            "rRNA",
...
.gen_splice <- function(reference_path, genome) {
    ...
    introns_search_MXE <- introns_search_MXE[
        ,
        c(
            "skip_coord", "gene_id", "Event", "transcript_id",
    ...
    setnames(introns_search_SE,
        ...
        new = c(
            "skip_coord", "skip_transcript_id", "skip_transcript_name",
            "skip_intron_number"
        )
    )
    introns_found_SE[introns_search_SE,
        on = "skip_coord",
        c(
            "gene_id_b", "skip_transcript_id",
            "skip_transcript_name", "skip_intron_number"
        ) :=
            list(
                get("i.gene_id"), get("i.skip_transcript_id"),
    ...
    introns_search_SE2 <- introns.skipcoord[
        ,
        c(
            "skip_coord", "gene_id", "Event", "transcript_id", "transcript_name",
            "intron_number"
        )
    ]
    introns_found_SE[introns_search_SE2,
        on = "skip_coord",
        c(
            "gene_id", "inc_coord_upst", "inc_transcript_id",
            "inc_transcript_name", "inc_intron_number"
        ) :=
            list(
                get("i.gene_id"), get("i.Event"), get("i.transcript_id"),
    ...
    introns_search_AFE_pos <- introns_search_AFE_pos[
        ,
        c(
            "seqnames", "intron_end", "strand", "Event", "gene_id",
    ...
    introns_search_AFE_neg <- introns_search_AFE_neg[
        ,
        c(
            "seqnames", "intron_start", "strand", "Event", "gene_id",
            "transcript_id", "transcript_name", "intron_number"
        )
    ]
    ...
    if (nrow(AS_Table) > 0) {
        ...
        AS_Table_SEarch.a <- candidate.introns.order[AS_Table_SEarch.a,
            on = "Event",
            c(
                "EventType", "EventID", "Event1a", "Event2a", "transcript_id",
        ...
        AS_Table_SEarch.b <- candidate.introns.order[AS_Table_SEarch.b,
            on = "Event",
            c(
                "EventType", "EventID", "Event1b", "Event2b", "transcript_id",
                "transcript_support_level", "is_protein_coding",
                "is_last_intron", "intron_number"
            )
        ]
        ...
        write.fst(
            as.data.frame(rbind(AS_Table_SEarch.a, AS_Table_SEarch.b)),
...
.gen_splice_proteins <- function(reference_path, genome) {
    ...
    seq <- substr(
        Upstream_A$seq[!is.na(Upstream_A$seqnames)],
    ...
    AS_Table.Extended[
        get("EventType") %in% c("MXE", "SE", "ALE", "A3SS"),
    ...
    # Trim sequence by phase
    seq <- substr(
        Upstream_B$seq[!is.na(Upstream_B$seqnames)],
    ...
    AS_Table.Extended[
        get("EventType") %in% c("MXE", "SE", "AFE", "A5SS"),
    ...
    Downstream_B[
        get("EventType") %in% c("SE", "AFE", "A5SS"),
        c("exon_number") := get("exon_number") + 1
    ]
    # left_join with Exons
    ...
    Casette.A[
        !is.na(get("phase_casette")) & is.na(get("upstr_seq")),
    ...
    Casette.A[
        !is.na(get("phase_casette")) & get("phase_casette") > 0 &
            !is.na(get("upstr_seq")),
    ...
    Casette.A[
        nchar(get("casette_seq_extended")) %% 3 > 0 & !is.na(get("Downstr_seq")),
        c("casette_seq_extended") := paste0(
            get("casette_seq_extended"),
    ...
    prot <- Biostrings::translate(as(seq, "DNAStringSet"))
    Casette.A[
        !is.na(get("casette_seq_extended")),
    ...
    Casette.B[, c("transcript_id", "exon_number") :=
        list(get("transcript_id_b"), get("intron_number_b"))]
    Casette.B[
        get("EventType") %in% c("MXE", "ALE", "A3SS"),
        c("exon_number") := get("exon_number") + 1
    ]
    ...
    # Add nucleotides from upstream and downstream
    Casette.B <- Upstream_B[Casette.B,
        on = "EventID",
        c("EventID", "phase_casette", "casette_seq", "seq")
    ]
    setnames(Casette.B, "seq", "upstr_seq")
    Casette.B <- Downstream_B[Casette.B,
        on = "EventID",
        c("EventID", "phase_casette", "casette_seq", "upstr_seq", "seq")
    ]
    ...
    Casette.B[
        ...
        c("casette_seq_extended") := paste0(
            substr(
                get("upstr_seq"), nchar(get("upstr_seq")) + 1 - get("phase_casette"),
                nchar(get("upstr_seq"))
            ),
            get("casette_seq_extended")
        )
...
DetermineNMD <- function(exon_list, intron_list, genome, threshold = 50) {
    ...
    msg = paste(
        "exon_list must be a GRanges, data.frame,",
        "or data.table coerce-able to a GRanges object"
    )
    )
    ...
    msg = paste(
        "intron_list must be a GRanges, data.frame,",
        "or data.table coerce-able to a GRanges object"
    )
    )
